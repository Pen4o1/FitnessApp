...
class StoreFoodLogRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'base_quantity' => ['required', 'numeric', 'min:0.001', 'max:10000'],
            'external_food_id' => ['required', 'string', 'max:100'],
            'external_source' => ['required', Rule::enum(FoodExternalSource::class)],
            'food_name' => ['required', 'string', 'max:255'],
            'brand_name' => ['nullable', 'string', 'max:255'],
            'calories_per_base' => ['required', 'integer', 'min:0'],
            'protein_g_per_base' => ['required', 'numeric', 'min:0'],
            'carbs_g_per_base' => ['required', 'numeric', 'min:0'],
            'fat_g_per_base' => ['required', 'numeric', 'min:0'],
        ];
    }
}
